...
#define LCD_ROWS    4
#define LCD_ADDRESS 0x27    // Atau 0x3F jika tidak tampil
...
// OLED PAGE
// =======================
int  oledPage = 0;              // 0 = halaman sensor, 1 = halaman kontrol, 2 = halaman status
...
// =======================
// UPDATE LCD 20x4 - 2 HALAMAN DENGAN DESIGN LEBIH BAGUS
// =======================
void updateLCD() {
  ...
  if (oledPage == 0) {
    // ===============================
    // HALAMAN 1: SENSOR & STATUS
    // ===============================
    
    // Baris 1: HEADER CENTERED
    // "[SENSOR DATA]" = 13 char, center = (20-13)/2 = 3.5 ≈ 4
    lcd.setCursor(4, 0);
    lcd.print("[SENSOR DATA]");

    // Baris 2: Temperature dan Humidity
    lcd.setCursor(0, 1);
    char tempBuf[10];
    ...
    lcd.setCursor(11, 1);
    char humBuf[10];
    dtostrf(humidity, 5, 1, humBuf);
    lcd.print("H:");
    lcd.print(humBuf);
    lcd.print("%");

    // Baris 3: Status dengan separator
    lcd.setCursor(0, 2);
    lcd.print("Mode:");
    ...
    lcd.setCursor(11, 2);
    if (fanStatus) lcd.print("FAN:[ON]");
    else           lcd.print("FAN:OFF ");

    // Baris 4: System Status
    lcd.setCursor(0, 3);
    lcd.print("W:");
    lcd.print(WiFi.isConnected() ? "OK " : "XX ");
    lcd.print("M:");
    lcd.print(mqttClient.connected() ? "OK " : "XX ");
    
    lcd.setCursor(10, 3);
    lcd.print("AL:");
    if (alarmStatus) lcd.print("[ON]");
    else            lcd.print("OFF ");

  } else {
    // ===============================
    // HALAMAN 2: KONTROL & INFO DETAIL
    // ===============================
    
    // Baris 1: HEADER CENTERED
    // "[CONTROL]" = 9 char, center = (20-9)/2 = 5.5 ≈ 6
    lcd.setCursor(6, 0);
    lcd.print("[CONTROL]");

    // Baris 2: MODE & FAN STATUS
    lcd.setCursor(0, 1);
    lcd.print("M:");
    if (mode == "AUTO") lcd.print("AUTO");
    else               lcd.print("MAN ");
    ...
    } else if (manualTimerSeconds > 0) {
      // Timer sudah diset
      if (manualTimerSeconds < 1000) {
        lcd.print(manualTimerSeconds);
        lcd.print("s");
      } else {
        lcd.print("ERR");
      }
    ...
    lcd.print("  ");  // Padding akhir

    // Baris 3: SETPOINT (format lebih aman)
    lcd.setCursor(0, 2);
    lcd.print("Set: ");
    // Format: XX.X/XX.X
    if (setpointOn < 10) lcd.print("0");
    lcd.print((int)setpointOn);
    lcd.print(".");
    lcd.print((int)(setpointOn * 10) % 10);
    lcd.print("/");
    if (setpointOff < 10) lcd.print("0");
    lcd.print((int)setpointOff);
    lcd.print(".");
    lcd.print((int)(setpointOff * 10) % 10);
    lcd.print("   ");

    // Baris 4: UPTIME & RUNTIME
    lcd.setCursor(0, 3);
    unsigned long upSeconds = millis() / 1000;
    unsigned long upHours = upSeconds / 3600;
    unsigned long upMins = (upSeconds % 3600) / 60;
    lcd.print("UP:");
    if (upHours < 10) lcd.print("0");
    lcd.print(upHours);
    lcd.print("h");
    if (upMins < 10) lcd.print("0");
    lcd.print(upMins);
    
    // Runtime
    String runtimeStr = formatRuntime(fanRuntimeSeconds);
    lcd.setCursor(11, 3);
    lcd.print(runtimeStr.substring(0, 8));
  }
}
...
void setup() {
  ...
  lcd.clear();
  lcd.setCursor(0, 0);
  lcd.print("ESP32 Fan Control");
  lcd.setCursor(0, 1);
  lcd.print("SHT31 + LCD 20x4");
  lcd.setCursor(0, 2);
  lcd.print("Initializing...");
  Serial.println("LCD 20x4 initialized OK");
  delay(2000);
